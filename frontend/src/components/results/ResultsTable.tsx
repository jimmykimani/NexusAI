...
export function ResultsTable() {
  const fully = useSearchStore((s) => s.fullyMatched)
  const partial = useSearchStore((s) => s.partiallyMatched)
  ...
  return (
    <section className="flex-1 flex flex-col min-h-0 min-w-0 bg-nexus-bg relative">
      ...
      <div className="leads-table-scroll flex-1 min-h-0 min-w-0 overflow-x-auto overflow-y-auto">
        {leads.length === 0 ? (
          <div className="min-h-full">
            {isSearching ? <SkeletonTable /> : <EmptyState />}
          </div>
        ) : (
...
function LeadsTable({ fully, partial }: { fully: Lead[]; partial: Lead[] }) {
  ...
  return (
    <table className="leads-data-table w-max min-w-full border-collapse text-sm">
      <thead className="sticky top-0 z-10 bg-nexus-bg">
